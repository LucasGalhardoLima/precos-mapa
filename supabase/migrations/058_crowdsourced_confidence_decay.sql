-- supabase/migrations/058_crowdsourced_confidence_decay.sql
--
-- Confidence decay for crowdsourced store_prices, per the decay curve in
-- specs/015-price-scanner/plan.md "Validation & trust": full weight for the
-- first ~48h, half by day 3-5, and dropped from results entirely once a
-- report is older than 14 days (a two-week-old single grocery-price scan
-- isn't useful ground truth). Computed at read time in the search RPC —
-- the stored store_prices.confidence value stays what the trigger set
-- (0.5 or 0.9); this only affects what's exposed/considered here.
...
BEGIN
  ...
  FROM (
    ...
    LEFT JOIN (
      ...
      -- staleness cutoff (see migration header).
      SELECT
        sp.product_id,
...
-- =============================================================================
-- get_product_prices — single-product version of the same merge, for the
-- product detail page. That page currently queries `promotions` directly
-- and only shows status='active' (not even last_price), so crowdsourced
-- prices are invisible there today. This RPC gives it the same three-tier
-- view search already has, without a broad rewrite of that screen's
-- existing promotions-fetch (kept separate — see mobile/app/product/[id].tsx).
-- =============================================================================
