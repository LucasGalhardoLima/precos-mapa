-- supabase/migrations/038_fix_search_null_location.sql
--
-- Fixes search_products_with_prices to handle null user_lat/user_lng gracefully.
-- When no location is available (e.g., permission not granted), the Haversine
-- filter was returning false for every store (null <= radius_km = null = false),
-- causing all searches to return zero results.
--
-- Fix: skip radius filtering when lat/lng are null and return null distance_km.
