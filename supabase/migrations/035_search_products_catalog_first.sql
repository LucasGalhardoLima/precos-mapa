-- supabase/migrations/035_search_products_catalog_first.sql
--
-- Rewrites search_products_with_prices to start from the products table
-- so Cosmos-seeded products appear in results even without any promotions.
--
-- Price tiers in order:
--   1. has active promo within radius        → "a partir de" (teal)
--   2. has last_price within radius          → "último preço" (grey)
--   3. catalog-only, has reference_price     → "preço sugerido" (grey)
--   4. catalog-only, no reference_price      → "preço sugerido —" (grey)
...
BEGIN
  ...
  FROM (
    ...
    LEFT JOIN (
      -- Aggregate all relevant promotions within radius per product
      SELECT
        pr.product_id,
    ...
    ORDER BY
      -- Tier 1: active promos → 2: last_price → 3: catalog+price → 4: catalog no price
      CASE
